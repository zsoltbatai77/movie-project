    ...
    if (in_array($kiterjesztes, $engedelyezett_kiterjesztesek)) {
        if ($_FILES["file"]["error"] === 0) {
            if ($_FILES["file"]["size"] <= 31457280) {
                $cel = "../kepek/cover/" . $_FILES["file"]["name"];
                if (move_uploaded_file($_FILES["file"]["tmp_name"], $cel)) {
                    rename($cel, "../kepek/cover/".$cutMovieName.".jpg");
                    $filename = $_FILES["file"]["name"];
                    $sql3 = "UPDATE movies SET coverimage = '/web-projekt/kepek/cover/$cutMovieName.jpg' WHERE title = '$movieName';";
